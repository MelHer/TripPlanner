                    ...
                    if($checking == "")
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                $transport_Types = select_Transport_Type();
                        ...
                        {
                            $id =   insert_Transport($_GET['id'],$_POST['place_Start'],$_POST['place_End'],$_POST['date_Start'],$_POST['date_End'],$_POST['time_Start'],$_POST['time_End'],$_POST['price'],$_POST['type'],$_POST['code'],$_POST['link'],$_POST['note'],$image_Checker);
                        }
                        
                        //Setting image file on server
...
function change_Transport()
{
    ...
    if(isset($_SESSION['user']) && !empty($_SESSION['user']))
    {
        ...
        if(isset($_GET['id']) && !empty($_GET['id']))
        {
            ...
            $transport = select_Transport($_SESSION['id'],$id_Transport);
            if(isset($transport) && !empty($transport))
            {
                ...
                if($_POST)
                {
                    ...
                    if($checking == "")
                    {
                        ...
                        if(isset($image_Checker)) //prevent creation if image not valid
                        {
                            update_Transport($transport['idTransport'],$_POST['place_Start'],$_POST['place_End'],$_POST['date_Start'],$_POST['date_End'],$_POST['time_Start'],$_POST['time_End'],$_POST['price'],$_POST['type'],$_POST['code'],$_POST['link'],$_POST['note'],$image_Checker);
                        }
        ...
        if(isset($m_Datas['date_Start']) && isset($m_Datas['date_End']) && strtotime($m_Datas['date_Start']) && strtotime($m_Datas['date_End']) && $m_Datas['date_Start'] <= $m_Datas['date_End'])
        {
            ...
            if(isset($m_Datas['time_Start']) && isset($m_Datas['time_End']) && (preg_match("/^(?:2[0-3]|[01][0-9]):[0-5][0-9]$/", $m_Datas['time_Start']) || $m_Datas['time_Start'] == null) && (preg_match("/^(?:2[0-3]|[01][0-9]):[0-5][0-9]$/", $m_Datas['time_End'])|| $m_Datas['time_End'] == null ))
            {
                ...
                {   
                    ...
                    $transport_Types = select_Transport_Type();
                    if(isset($m_Datas['type']) && $m_Datas['type'] > 0 && $m_Datas['type'] <= count($transport_Types))
                    {
                        //Reservation code
